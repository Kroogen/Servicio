...
namespace _Scripts.Quetzal
{
    public class TestZarit : MonoBehaviour
    {
        ...
        {
            posElement++;
            if (posElement < Constantes.TOTAL_PREGUNTAS_ZARIT)
            {
                transform.position = new Vector3(
                    Constantes.TESTZARIT_X[posElement], 
                    Constantes.TESTZARIT_Y[posElement],
                    Constantes.TESTZARIT_Z[posElement]);
        ...
        private void OnMouseDown()
        {
            if (!disponible)
            {
                xObjetivo = Constantes.TESTZARIT_X[posElement];
                yObjetivo = Constantes.TESTZARIT_Y[posElement];
                zObjetivo = Constantes.TESTZARIT_Z[posElement];
                ...
                UI.gameObject.GetComponent<Animator>().Play("Abrir");
                disponible = true;
